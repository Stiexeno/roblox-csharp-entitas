...
namespace RobloxCSharp.Extensions.Entitas
{
	...
	{
		// Emits members on `partial class {Ctx}Entity` instead of static
		// extension methods. Matches Jenny's output 1:1 so frozen-feast
		// source compiles unchanged (`entity.health.Value`, `entity.isPlayer = true`).
		public static string Emit(ContextModel ctx)
		{
			StringBuilder sb = new();
			sb.AppendLine("// <auto-generated/> roblox-csharp-entitas");
			...
			foreach (ComponentModel c in ctx.Components)
			{
				...
				if (c.IsFlag) EmitFlag(sb, c, lookup);
				else EmitValue(sb, c, lookup);
				sb.AppendLine();
			}
			sb.AppendLine("}");
			return sb.ToString();
		...
		private static void EmitFlag(StringBuilder sb, ComponentModel c, string lookup)
		{
		...
		private static void EmitValue(StringBuilder sb, ComponentModel c, string lookup)
		{
	...
	internal static class MatcherTemplate
	{
		public static string Emit(ContextModel ctx)
		{
			...
			for (int i = 0; i < ctx.Components.Count; i++)
			{
				ComponentModel c = ctx.Components[i];
			...
			// Route through the non-generic Matcher static class — the
			// generic-class-as-qualifier form (`Matcher<T>.AllOf(...)`)
			// doesn't lower today. Same shape Jenny emits.
