...
namespace RobloxCSharp.Extensions.Entities
{
	...
	// Maps C# primitive type names to the EntitiesFieldType.X constant
	// identifier used by the buffer-wire schema. Codegen emits these in
	// both ServerReplication and ClientReplication so the runtime knows
	// how to pack/unpack each [Replicated] component's fields.
	internal static class ReplicationFieldTypeMap
	{
		public static string ToCode(string typeFullName)
		{
			return typeFullName switch
			{
				"int" => "EntitiesFieldType.I32",
				"float" => "EntitiesFieldType.F32",
				"double" => "EntitiesFieldType.F64",
				"bool" => "EntitiesFieldType.Bool",
				"string" => "EntitiesFieldType.String",
				"byte" => "EntitiesFieldType.U8",
				"ushort" => "EntitiesFieldType.U16",
				"uint" => "EntitiesFieldType.U32",
				"sbyte" => "EntitiesFieldType.I8",
				"short" => "EntitiesFieldType.I16",
				_ => throw new System.Exception($"Replicated component field type not supported by buffer wire: '{typeFullName}'. Supported: int, float, double, bool, string, byte, ushort, uint, sbyte, short."),
			};
		}

		public static string SchemaArrayExpr(ComponentModel c)
		{
			if (c.IsFlag) return "new int[0]";
			string[] codes = c.Fields.Select(f => ToCode(f.TypeFullName)).ToArray();
			return $"new int[] {{ {string.Join(", ", codes)} }}";
		}
	}

	// Per-context server replication bootstrap. Registers schemas + a
	// snapshotter; the runtime calls the snapshotter when a new player's
	// client fires the Ready event. The snapshotter walks every live
	// entity, checks each replicated component's HasX, and enqueues a
	// Set op via QueueSet — which the runtime packs into a per-player
	// snap buffer (because _snapshotMode is set) and fires to that
	// player. Order is GetEntities() order then codegen order.
	//
	...
	// guard on IsServer internally, so unconditional construction is
	...
	internal static class ServerReplicationTemplate
	{
		public static string Emit(ContextModel ctx, List<ComponentModel> replicated)
		{
			...
			// Per-component schema registration — runtime uses these to pack
			// each QueueSet into the per-context buffer.
			foreach (ComponentModel c in replicated)
			{
				string lookup = $"{ctx.Name}ComponentsLookup.{c.TypeName}";
				sb.AppendLine($"\t\tEntitiesReplication.RegisterComponentSchema(\"{ctx.Name}\", {lookup}, {ReplicationFieldTypeMap.SchemaArrayExpr(c)});");
			}
			sb.AppendLine($"\t\tEntitiesReplication.RegisterSnapshotter(\"{ctx.Name}\", Snapshot);");
			sb.AppendLine("\t}");
			sb.AppendLine();

			// Snapshot — runs inside the runtime's _snapshotMode flag, so
			// QueueSet calls land in the per-player snap buffer instead of
			// the shared tick buffer. Each QueueSet has the same wire shape
			// as a tick-time Set, so the client's applier handles them
			// through the same path.
			sb.AppendLine("\tprivate void Snapshot()");
			sb.AppendLine("\t{");
			...
			foreach (ComponentModel c in replicated)
			{
				...
				if (c.IsFlag)
				{
					sb.AppendLine($"\t\t\tif (e.Is{c.TypeName}) EntitiesReplication.QueueSet(\"{ctx.Name}\", {lookup}, e.creationIndex);");
				}
				else
				{
					bool unwrap = c.Fields.Count == 1 && c.Fields[0].Name == "Value";
					...
					if (unwrap)
					{
						sb.AppendLine($"\t\t\t\tEntitiesReplication.QueueSet(\"{ctx.Name}\", {lookup}, e.creationIndex, e.{c.TypeName});");
					}
					else
					{
						sb.AppendLine($"\t\t\t\t{c.FullName} comp = e.{c.TypeName};");
						string fieldArgs = string.Join(", ", c.Fields.Select(f => $"comp.{f.Name}"));
						sb.AppendLine($"\t\t\t\tEntitiesReplication.QueueSet(\"{ctx.Name}\", {lookup}, e.creationIndex, {fieldArgs});");
					}
					sb.AppendLine("\t\t\t}");
				}
			}
	...
	// Per-context client replication handler. Registers schemas + per-
	// component typed appliers/removers with the runtime, then calls
	// SubscribeClient to wire OnClientEvent into the runtime's unpack-
	// and-dispatch loop. The runtime walks each received buffer op-by-op:
	// reads opcode + componentIdx + entityId + fields-per-schema, then
	// calls the matching Apply{X}(serverId, ...fields) or
	// Remove{X}(serverId) on this instance.
	//
	// ShouldEmit() returns false on the client, so the AddX/ReplaceX/
	// RemoveX calls made inside appliers never echo back over the wire.
	//
	...
	// dropped (snapshot for that entity hasn't arrived yet, or the
	// entity was already destroyed locally).
	internal static class ClientReplicationTemplate
	{
		public static string Emit(ContextModel ctx, List<ComponentModel> replicated)
		{
			...
			sb.AppendLine("\t{");
			sb.AppendLine("\t\t_context = context;");
			// Schema registration — runtime uses these to unpack each
			// received op's fields before invoking the applier.
			foreach (ComponentModel c in replicated)
			{
				string lookup = $"{ctx.Name}ComponentsLookup.{c.TypeName}";
				sb.AppendLine($"\t\tEntitiesReplication.RegisterComponentSchema(\"{ctx.Name}\", {lookup}, {ReplicationFieldTypeMap.SchemaArrayExpr(c)});");
			}
			// Per-component appliers + removers.
			foreach (ComponentModel c in replicated)
			{
				string lookup = $"{ctx.Name}ComponentsLookup.{c.TypeName}";
				sb.AppendLine($"\t\tEntitiesReplication.RegisterClientApplier(\"{ctx.Name}\", {lookup}, Apply{c.TypeName});");
				sb.AppendLine($"\t\tEntitiesReplication.RegisterClientRemover(\"{ctx.Name}\", {lookup}, Remove{c.TypeName});");
			}
			// SubscribeClient wires OnClientEvent + fires the Ready ping
			// once (idempotent across multiple contexts).
			sb.AppendLine($"\t\tEntitiesReplication.SubscribeClient(\"{ctx.Name}\");");
			sb.AppendLine("\t}");
			sb.AppendLine();

			// `out var x` doesn't lower today (DeclarationExpressionSyntax
			// gap), so use the ContainsKey + indexer pattern.
			...
			// Per-component Apply / Remove — called by the runtime after it
			// unpacks the buffer using the registered schema. Apply picks
			// Add vs Replace by HasX so server re-sends (snapshot,
			// re-sync) stay idempotent.
			foreach (ComponentModel c in replicated)
			{
				if (c.IsFlag)
				{
					sb.AppendLine($"\tprivate void Apply{c.TypeName}(int serverId)");
					sb.AppendLine("\t{");
					sb.AppendLine($"\t\tGetOrCreate(serverId).Is{c.TypeName} = true;");
					sb.AppendLine("\t}");
					sb.AppendLine();
					sb.AppendLine($"\tprivate void Remove{c.TypeName}(int serverId)");
					sb.AppendLine("\t{");
				...
				else
				{
					...
					sb.AppendLine($"\tprivate void Apply{c.TypeName}(int serverId, {fieldParams})");
					sb.AppendLine("\t{");
					sb.AppendLine($"\t\t{entityType} e = GetOrCreate(serverId);");
					sb.AppendLine($"\t\tif (e.Has{c.TypeName}) e.Replace{c.TypeName}({fieldArgs});");
					sb.AppendLine($"\t\telse e.Add{c.TypeName}({fieldArgs});");
					sb.AppendLine("\t}");
					sb.AppendLine();
					sb.AppendLine($"\tprivate void Remove{c.TypeName}(int serverId)");
					sb.AppendLine("\t{");
